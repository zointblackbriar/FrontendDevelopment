...
var drag_line = outer.append('svg:path')
    .attr('class', 'link dragline hidden')
    .attr('d', 'M0,0L0,0');

// handles to link and node element groups
var path = outer.append('svg:g').selectAll('path'),
    circle = outer.append('svg:g').selectAll('g');
...
function update() {
    ...
    if(clearFlag == true && mouseDownFlag != true)
    {
        //When clicked ClearDesigner, it will clear nodes and links
        nodes = [];
        links = [];
    }
    //load button activated
    if(loadFlag == true)
    {
        ...
        d3.json("nodes.json", function(error, json) {
            if(error)
            {
                console.log('nodes.json error', error);
                throw error;
            }
            console.log("d3.json");
    ...
    g.append('svg:text')
        ...
        .attr('y', 4)
        .attr('class', 'id')
        .text(function(d) { return "Service" + d.id; });
...
function mousedown() {
    ...
    outer.classed('active', true);

    if(d3.event.ctrlKey || mousedown_node || mousedown_link) return;
    console.log("mousedown");
        //console.log("key code 18");
...
function sendNodes()
{
    jQuery(document).ready(function($)
    {
        $.ajax({
            ...
            data:JSON.stringify({links: links, nodes: nodes}),
